...
import com.example.appointment.MapSearchAdapter
...
class ScheduleMapActivity : AppCompatActivity(),OnMapReadyCallback,MapSearchAdapter.OnItemClickListener{
    lateinit var binding: ActivityScheduleMapBinding
    lateinit var adapter : MapSearchAdapter
    ...
    fun setObserve(){
        mainViewmodel.showKeywordList.observe(this){//왜 이거 이럼?보이는거에 따라? 리스트변경에 따르는게 더 좋지 않을까?
            if(it){
                ...
                adapter = MapSearchAdapter(mainViewmodel.searchKewordList.value!!,this)
